...
$sql = "SELECT * FROM tbl_senior_examinations WHERE exam_id = '$exam_id' AND category = '$mycategory' AND status = 'Active'";
...
$sql = "SELECT * FROM tbl_senior_records WHERE student_id = '$myid'";
$result = $conn->query($sql);

if ($result->num_rows > 0) {

    while($row = $result->fetch_assoc()) {
    header("location:./take-senior-assessment.php?id=$exam_id");
    }
} else {
$myname = "$myfname $mylname";
$recid = 'RS'.get_rand_numbers(14).'';

$sql = "INSERT INTO tbl_senior_records (record_id, student_id, student_name, category, subject, passmark,exam_name, exam_id, score, status, next_retake, date)
                    ...
                    <div class="row">
                                <div class="panel panel-white">
                                    <div class="panel-body">
                                       ...
                                       <form action="pages/submit_senior_assessment.php" method="POST" name="quiz" id="quiz_form" >
                                            <div class="tab-content">
											<?php 
											include '../database/config.php';
											$sql = "SELECT * FROM tbl_senior_questions WHERE exam_id = '$exam_id'";
                                            $result = $conn->query($sql);
                                            ...
                                            if ($pathnn4 == NULL) {
                                                ...
                                                $pathnn4 = '<img src="data:image/jpeg;base64,'.base64_encode($pathnn4).'" width="700" height="500" alt=""/>';	
                                            }
                                            ...
                                            '.$pathnn4.'
                                             <p><b>'.$qno.'.</b> '.$qs.'</p>
											 <p>A <input type="radio" name="an'.$qno.'"   value="'.$op1.'"> '.$op1.'</p>
											 <p>B <input type="radio" name="an'.$qno.'"   value="'.$op2.'"> '.$op2.'</p>
											 <p>C <input type="radio" name="an'.$qno.'"   value="'.$op3.'"> '.$op3.'</p>
                                             <p>D <input type="radio" name="an'.$qno.'"   value="'.$op4.'"> '.$op4.'</p>
                                             <p>E <input type="radio" name="an'.$qno.'"   value="'.$op5.'"> '.$op5.'</p>
											 <input type="hidden" name="qst'.$qno.'" value="'.base64_encode($qs).'">
...
// $sql = "INSERT INTO tbl_users_answers (question_id, exam_id, exam_idx, userans, correctans)
// VALUES ('$recid','$myid','$exam_idx','$userans','$answer')";
...
// }
								...
								<br><input style="float:right" onclick="return confirm('Are you sure you want to submit your assessment ?')" class="btn btn-success" type="submit" value="Submit Assessment">
											</form>
                                    </div>
                                </div>  
                    </div>
